...
if [[ "${SAFE}" -eq 1 ]]; then
  ...
  run "${RUNNER}" --chat "${CHAT}" --display "${DISPLAY_VALUE}" --xauthority "${XAUTHORITY_VALUE}" --path "${WORK_DIR}/wechat-send-file-selftest.md" --print-only
  run "${RUNNER}" --chat "${CHAT}" --display "${DISPLAY_VALUE}" --xauthority "${XAUTHORITY_VALUE}" --documents-root "${WORK_DIR}/documents" --print-only
  log "safe 模式完成"
  exit 0
fi

run "${RUNNER}" --chat "${CHAT}" --display "${DISPLAY_VALUE}" --xauthority "${XAUTHORITY_VALUE}" --path "${WORK_DIR}/wechat-send-file-selftest.md"

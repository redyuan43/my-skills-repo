...
readonly GPU_TEMP_THRESHOLD_C="${GPU_TEMP_THRESHOLD_C:-65}"
...
set_auto_mode() {
  ...
  systemctl start "${NVFANCONTROL_SERVICE}"
  current_mode="auto"
  log "GPU temperature < ${GPU_TEMP_THRESHOLD_C}C, restored automatic fan control."
}

cleanup() {
  set +e
  systemctl start "${NVFANCONTROL_SERVICE}" >/dev/null 2>&1
}
...
main() {
  ...
  log "Watching ${gpu_zone_dir}/temp with threshold ${GPU_TEMP_THRESHOLD_C}C."

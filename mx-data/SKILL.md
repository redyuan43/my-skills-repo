---
name: mx-data
description: 基于东方财富妙想 API 的金融数据查询工具。Use when the user needs current or historical A-share/HK-share market data, financial indicators, company basics, shareholders, management, business segments, related-party relationships, or authoritative Eastmoney data via MX_APIKEY.
---
...
python3 scripts/mx_data.py "贵州茅台今日收盘价 涨跌幅"
python3 scripts/mx_data.py "宁德时代主力资金流向"

# 2. 历史行情数据查询
python3 scripts/mx_data.py "贵州茅台近五年年报收盘价"
python3 scripts/mx_data.py "比亚迪近一年每个交易日的开盘价收盘价成交量"

# 3. 财务数据查询
python3 scripts/mx_data.py "贵州茅台近三年净利润 营业收入"
python3 scripts/mx_data.py "东方财富每股收益 净资产收益率 近五年"
python3 scripts/mx_data.py "万科A资产负债率 毛利率 近三年"

# 4. 上市公司基本信息
python3 scripts/mx_data.py "比亚迪公司简介 主营业务 成立时间"
python3 scripts/mx_data.py "贵州茅台董事长是谁 总股本多少"

# 5. 股东信息
python3 scripts/mx_data.py "贵州茅台十大股东"
python3 scripts/mx_data.py "比亚迪机构持股比例"

# 6. 板块/指数行情
python3 scripts/mx_data.py "沪深300指数最新点位 涨跌幅"
python3 scripts/mx_data.py "新能源板块成分股平均涨跌幅"

# 7. 指定输出目录（可选）
python3 scripts/mx_data.py "贵州茅台近五年年报" /path/to/output
```
...
脚本依赖 `requests`、`pandas` 和 `openpyxl` 库。首次运行报 `ModuleNotFoundError` 时，优先在项目 venv 里安装：
...
### 1. 非确定性行为 (Non-deterministic API)

妙想API的自然语言解析层**不是确定性的**——完全相同的查询语句在不同时间调用可能返回不同结果：
- 某次调用返回2个dataTableDTOList block，下次调用同一个查询返回空的 searchDataResultDTO
- **建议**：对关键查询**重试2-3次**，不要假设一次调用失败就是永久失败

### 2. 2025年年报查询最佳实践

通过大量测试验证，以下查询格式成功率最高：

```
# ✅ 推荐格式：股票名 + 代码 + 年份范围 + "年报" + 指标
"贵州茅台 600519.SH 2023 2024 2025 年报 净利润 营业收入 每股收益"
"宁德时代 300750 2023 2024 2025 年报 净利润 营业收入 ROE"
"比亚迪 002594.SZ 2023 2024 2025 年报 净利润 营业收入 每股收益"
"中国移动 600941 2021 2022 2023 2024 2025 年报 净利润 营业收入"

# ✅ 简化格式（部分股票适用）
"海康威视 002415 年报 2025"
"腾讯控股 00700 2025 年报 营收 净利"
"五粮液 000858 2025 年报"

# ❌ 低效格式（易返回空）
"贵州茅台2025年年报净利润"          # 无空格无代码，NLP解析失败率高
"600519.SH 2025年报"               # 只有代码和年份，缺少股票名称
"贵州茅台近五年年报"                # 时间范围模糊，易被解析为行情数据
```

**格式要点**:
- 必须包含**股票中文名 + 代码**（带市场后缀.SH/.SZ更佳）
- 年份用空格分隔显式列出（`2023 2024 2025`）优于"近三年"等模糊表述
- 指标名放在最后，用空格分隔

### 3. 覆盖范围缺口（2025年年报）

以下A股蓝筹的2025年年报数据**通过妙想API无法获取**（已验证）:
- 招商银行 (600036.SH)
- 中国平安 (601318.SH)
- 恒瑞医药 (600276.SH)

原因可能是东方财富数据库对这些特定股票的年度指标未通过自然语言接口暴露。如需此类数据，建议 fallback 到 mx-search 或直接从原始JSON调试。

### 4. 港股支持有限

腾讯控股 (00700.HK) 勉强可通过`"腾讯控股 00700 2025 年报 营收 净利"`获取，但大多数港股不支持。验证前先用简单行情查询测试连通性。
...
当脚本报"接口返回中无 dataTableDTOList"时，**不一定是API挂了**——可能只是当前查询没有匹配数据。正确诊断方法：

```python
import json, sys
sys.path.insert(0, '/path/to/mx-data')
from mx_data import MXData
c = MXData()
resp = c.query('你的查询')

# 检查API返回结构
srd = resp.get('data',{}).get('data',{}).get('searchDataResultDTO',{})
if not srd:
    print('searchDataResultDTO为空，API未识别此查询')
else:
    dtl = srd.get('dataTableDTOList') or []
    print(f'找到 {len(dtl)} 个数据block')
```
...
| **searchDataResultDTO为空但success=true** | 查询条件NLP未解析到匹配数据，非API错误 | 放宽查询条件，改用推荐格式重试2-3次 |
| **"接口返回中无 dataTableDTOList"** | parse_result()遇到空的searchDataResultDTO | 这是脚本的误导性报错，见上方诊断技巧 |
...
详情见 `references/2025-annual-report-test-results.md` — 2025年5月17日测试的详细结果，包含各股票查询格式、返回数据样例、覆盖缺口。

...
description: 对 `my-skills-repo` 里的顶层 skills 做静态质量检查和评分，覆盖结构、文档、可执行性与风险模式扫描。适用于想快速发现缺失 `selftest.sh`、危险命令、脚本语法问题或文档不完整的场景。
---
...
- 默认只做静态检查，不执行破坏性命令
- 会检查结构、文档关键词、shell 语法、`selftest.sh` 可执行位和风险命令模式

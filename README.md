...
- `scripts/selftest.sh` - 真实/无副作用自检脚本，用于验证提 key、解密链路与可选微信回执

### 32. codex-connectivity-fix
**功能：** 修复 Linux / SSH 环境下 Codex CLI 的启动与 OpenAI 连通性问题
**用途：** 当 `codex` 命令找不到、启动报 `Unexpected reserved word`、系统默认 Node 太老、`api.openai.com` 直连失败、或本地 `xray/v2ray/clash` 已运行但 Codex 没继承代理环境时使用
**文件：**
- `SKILL.md` - 技能说明文档（故障特征、标准工作流、经验规则与验证步骤）

### 33. lmstudio-llmster-gui-reconcile
**功能：** 修复 Linux 上 `llmster` 已升级但 LM Studio GUI 与桌面图标仍停留在旧 AppImage 的分叉状态，并统一软链、桌面入口与旧包清理
**用途：** 当执行 `curl -fsSL https://lmstudio.ai/install.sh | bash` 后，`lms daemon up` 是新的，但桌面图标仍打开旧版 LM Studio，或需要把 `~/.local/bin/lmstudio`、`lmstudio.desktop`、最新 GUI AppImage 和旧包清理统一收口时使用
**文件：**
- `SKILL.md` - 技能说明文档（适用边界、工作流、验证口径与经验规则）
- `scripts/reconcile_lmstudio_gui.sh` - 检查 `llmster` / GUI 分叉、下载最新 GUI AppImage、切换软链、重写桌面入口并可选清理旧包
- `scripts/selftest.sh` - 无副作用自检脚本，验证脚本入口与 latest 下载链接解析是否正常

### 34. wechat-chat-watch
...
### 71. android-tablet-sunshine-moonlight-ubuntu
...
### 73. linux-proxy-socks5-socket-fix
...
### 74. lan-ssh-alias-and-key-setup
...
**功能：** 在 Linux 桌面状态栏配置 CPU、GPU/RAM 和 IPv4 监控；支持 XFCE 面板原生插件和 Ubuntu/GNOME 顶栏多项 AppIndicator
**用途：** 当需要把同一套状态栏监控快速部署到本机或多台 Linux 桌面设备（含通过 SSH 远程执行 XFCE）时使用
**文件：**
- `SKILL.md` - 技能说明文档（适用边界、Ubuntu/GNOME 与 XFCE 部署流程、验证命令）
- `scripts/setup-ubuntu-statusbar-monitors.sh` - Ubuntu/GNOME 顶栏多项 AppIndicator 部署脚本（写入用户级小程序和自启动项）
...
### 87. mx-data
**功能：** 通过东方财富妙想 API 查询行情、财务、股东、关联关系等金融数据
**用途：** 当需要 A 股/港股当前行情、历史行情、财务指标、公司基本信息、股东结构等权威数据时使用
**文件：**
- `SKILL.md` - 技能说明文档（查询场景、调用示例、输出文件与异常处理）
- `scripts/mx_data.py` - 妙想金融数据查询脚本，输出 Excel、描述文件和原始 JSON
- `agents/openai.yaml` - skill 的 UI 元数据

### 88. mx-search
**功能：** 通过东方财富妙想搜索金融资讯、公告、研报、政策和事件解读
**用途：** 当需要涉及时效性的金融信息、个股公告、机构观点、行业政策或市场异动原因时使用
**文件：**
- `SKILL.md` - 技能说明文档（资讯搜索流程、查询示例和错误处理）
- `scripts/mx_search.py` - 妙想资讯搜索脚本，输出纯文本和原始 JSON
- `agents/openai.yaml` - skill 的 UI 元数据

### 89. mx-xuangu
**功能：** 通过东方财富妙想按行情、财务、行业、板块或指数条件智能筛选股票
**用途：** 当需要构建 A 股候选池、按 PE/ROE/增长率/行业等条件筛选标的，或查询板块/指数成分股时使用
**文件：**
- `SKILL.md` - 技能说明文档（选股条件、调用示例、输出 CSV 和诊断说明）
- `scripts/mx_xuangu.py` - 妙想智能选股脚本，输出 CSV、描述文件和原始 JSON
- `agents/openai.yaml` - skill 的 UI 元数据

### 90. mx-zixuan
**功能：** 查询、添加或删除东方财富妙想自选股
**用途：** 当用户明确需要查看自选股列表、把标的加入自选，或从自选中移除标的时使用
**文件：**
- `SKILL.md` - 技能说明文档（自选股查询、添加、删除流程）
- `scripts/mx_zixuan.py` - 妙想自选股管理脚本，输出 CSV 和原始 JSON
- `agents/openai.yaml` - skill 的 UI 元数据

### 91. mx-moni
**功能：** 查询和操作东方财富妙想模拟组合，包括持仓、资金、委托、买卖和撤单
**用途：** 当用户明确要求模拟盘查询、模拟交易、策略验证或模拟组合复盘发帖时使用；不用于真实资金交易
**文件：**
- `SKILL.md` - 技能说明文档（模拟组合接口、自然语言意图和安全边界）

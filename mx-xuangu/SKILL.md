---
name: mx-xuangu
description: 基于东方财富妙想智能选股 API 的 A 股筛选 skill。Use when the user wants to screen stocks by market, valuation, financial, industry, sector, index constituent, dividend, growth, ROE, or combined natural-language conditions via MX_APIKEY.
---
...
- **默认输出目录**: `~/.mx/mx-xuangu/output/`（自动创建）
- **输出文件名前缀**: `mx_xuangu_`
- **输出文件**:
...
# 1. 按行情条件筛选
python3 scripts/mx_xuangu.py "今日涨幅大于2%的A股"
python3 scripts/mx_xuangu.py "成交量大于10亿的股票"
python3 scripts/mx_xuangu.py "市盈率小于20并且市净率小于2"
python3 scripts/mx_xuangu.py "股价在10元到20元之间"

# 2. 按财务条件筛选
python3 scripts/mx_xuangu.py "净利润增长率大于30%的股票"
python3 scripts/mx_xuangu.py "净资产收益率大于15%的公司"
python3 scripts/mx_xuangu.py "股息率大于3%的银行股"

# 3. 指定行业/板块
python3 scripts/mx_xuangu.py "新能源板块市盈率小于30的股票"
python3 scripts/mx_xuangu.py "白酒板块涨幅大于1%的股票"
python3 scripts/mx_xuangu.py "半导体行业毛利率大于40%的公司"

# 4. 指数成分股筛选
python3 scripts/mx_xuangu.py "沪深300成分股中分红率最高的10只股票"
python3 scripts/mx_xuangu.py "创业板成分股中市盈率小于30"

# 5. 组合条件
python3 scripts/mx_xuangu.py "价格小于20元 市盈率小于20 涨幅大于1% A股"
python3 scripts/mx_xuangu.py "ROE大于15% 净利润连续三年增长"

# 也可以使用 --query 参数
python3 scripts/mx_xuangu.py --query "市盈率小于20的银行股"
```
...
脚本执行后会：
1. 在终端输出统计信息（行数、文件位置）
2. 自动创建输出目录 `~/.mx/mx-xuangu/output/`

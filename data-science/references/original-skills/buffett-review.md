---
name: buffett_review
description: "Activates Warren Buffett's complete investment thinking system. Triggers on stock analysis, investment evaluation, financial report interpretation, moat/management assessment, buy/hold/sell decisions. NOT auto-generated alongside cfo-deep-dive — only on explicit user request like '用巴菲特视角分析', '巴菲特怎么看', '生成巴菲特报告'. 8-question quick filter, 5-moat framework, owner earnings valuation, margin-of-safety tiers. Reads reference files for deep analysis."
---
...
> **Read reference files with skill_view(name='buffett_review', file_path='references/<filename>')**\n> Construction: `skill_view(name='buffett_review', file_path='references/03-business-moat.md')`
...
File naming (same version as the CFO trio):
```
buffett_view_[股票代码]_[公司名]_[日期]_V[版本].md
```

Stored alongside the other reports at:
```
~/.hermes/skills/data-science/financial-screener/reports/
```
...
**⚠️ Calculation Discipline (from 2026-05-03 BZ postmortem):**
All financial calculations — currency conversions, ratios, valuations — must be executed via Python `execute_code` or terminal commands. Mental arithmetic is prohibited. Every figure must carry its currency prefix (¥/$) and unit (亿/万). When a computed ratio exceeds reasonable bounds (e.g., net cash/market cap > 80%), pause and verify the currency consistency before proceeding.

---
name: mx-zixuan
description: 东方财富妙想自选股管理 skill。Use when the user explicitly wants to query, add, or delete stocks in their Eastmoney/MX watchlist using MX_APIKEY and natural-language or command-style instructions.
---
...
- **默认输出目录**: `~/.mx/mx-zixuan/output/`（自动创建）
- **输出文件名前缀**: `mx_zixuan_`
- **输出文件**:
...
```bash
# 明确命令
python3 scripts/mx_zixuan.py query

# 自然语言查询
python3 scripts/mx_zixuan.py "查询我的自选股列表"
python3 scripts/mx_zixuan.py "我的自选"
python3 scripts/mx_zixuan.py "看一下自选"
```

### 2. 添加股票到自选股
```bash
# 明确命令
python3 scripts/mx_zixuan.py add "贵州茅台"
python3 scripts/mx_zixuan.py add "300059"

# 自然语言
python3 scripts/mx_zixuan.py "把贵州茅台添加到我的自选股列表"
python3 scripts/mx_zixuan.py "加入自选 比亚迪"
```

### 3. 删除自选股
```bash
# 明确命令
python3 scripts/mx_zixuan.py delete "贵州茅台"

# 自然语言
python3 scripts/mx_zixuan.py "把贵州茅台从我的自选股列表删除"
python3 scripts/mx_zixuan.py "删除自选 万科A"
```

---
name: mx-search
description: 基于东方财富妙想搜索能力的金融资讯搜索 skill。Use when the user needs timely financial news, announcements, research reports, policies, trading rules, event explanations, institution views, or external evidence from Eastmoney sources via MX_APIKEY.
---
...
- **默认输出目录**: `~/.mx/mx-search/output/`（自动创建）
- **输出文件名前缀**: `mx_search_`
- **输出文件**:
...
# 1. 个股相关资讯
python3 scripts/mx_search.py "东方财富最新公告"
python3 scripts/mx_search.py "贵州茅台最新研报"
python3 scripts/mx_search.py "比亚迪机构观点汇总"

# 2. 行业/板块新闻
python3 scripts/mx_search.py "人工智能板块近期新闻"
python3 scripts/mx_search.py "新能源汽车产业政策最新解读"

# 3. 宏观经济与市场分析
python3 scripts/mx_search.py "美联储加息对A股影响分析"
python3 scripts/mx_search.py "今日大盘异动原因分析"
python3 scripts/mx_search.py "北向资金最新流向解读"

# 4. 个股事件
python3 scripts/mx_search.py "贵州茅台分红派息实施公告"
python3 scripts/mx_search.py "宁德时代定增预案解读"

# 5. 交易规则
python3 scripts/mx_search.py "科创板交易涨跌幅限制"
python3 scripts/mx_search.py "新股申购规则"

# 6. 指定输出目录（可选）
python3 scripts/mx_search.py "格力电器最新研报" /path/to/output
```
...
2. 自动创建输出目录 `~/.mx/mx-search/output/`
3. 保存纯文本提取结果到 `.txt` 文件
4. 保存原始 JSON 响应供二次处理

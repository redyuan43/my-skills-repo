...
WORK_DIR="$(mktemp -d -t md-pdf-selftest.XXXXXX)"
...
check_file "SKILL.md"
check_file "scripts/md_to_pdf.sh"
check_file "references/output_gate.md"

bash -n "$ROOT/scripts/md_to_pdf.sh"
...
This is a minimal markdown-to-pdf smoke document.
MD

export npm_config_cache="$WORK_DIR/npm-cache"
"$ROOT/scripts/md_to_pdf.sh" "$WORK_DIR/input.md" "$WORK_DIR/output.pdf" >/dev/null

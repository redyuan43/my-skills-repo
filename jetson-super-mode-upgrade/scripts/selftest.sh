...
check_file "SKILL.md"
check_file "scripts/check_jetson_super_mode.py"
...
python3 -m py_compile "$ROOT/scripts/check_jetson_super_mode.py"
python3 -m json.tool "$ROOT/eval/val/items.json" >/dev/null

check_contains "SKILL.md" "Rollback|回滚"
check_contains "SKILL.md" "Safety Rules"
...
for cmd in nvbootctrl nvpmodel jetson_clocks; do
